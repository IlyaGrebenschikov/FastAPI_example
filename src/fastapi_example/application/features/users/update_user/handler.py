...
from fastapi_example.application.interfaces.database.repositories import IUsersRepository, TUpdateUser
from fastapi_example.application.exceptions.http_exceptions import NotFoundError, ConflictError
...
class UpdateUserHandler:
    def __init__(self, repository: IUsersRepository, transaction_manager: ITransactionManager):
        self._repository = repository
        self._transaction_manager = transaction_manager

    async def execute(
        self, cmd: UpdateUserCommand
    ) -> User:
        async with self._transaction_manager:
            ...
            raw_data = {k: v for k, v in asdict(cmd).items() if v is not None and k != "user_id"}
